...
public class ImageTool {
	public static org.eclipse.swt.graphics.Image resize (org.eclipse.swt.graphics.Image image, Dimension maxSize) {
		ImageData data = image.getImageData();
		return resize(image,computeScale(data, maxSize));
	}
	
	public static float computeScale(ImageData data, Dimension maxSize){
		float scale = Math.min(
				data.width > maxSize.width ? (float) maxSize.width
						/ data.width : 1,
				data.height > maxSize.height ? (float) maxSize.height
						/ data.height : 1);
		return scale;
	}
