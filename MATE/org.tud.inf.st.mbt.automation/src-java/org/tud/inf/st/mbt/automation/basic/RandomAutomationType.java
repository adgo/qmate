...
public class RandomAutomationType extends ManualAutomationType {

	private static RandomAutomationType instance;
	private static final String NAME = "Random Automation";
	private static Random rand = new Random();
	private static Logger logger = Logger.getLogger(NAME);

	private RandomAutomationType() {
		super(NAME);
	}

	public static RandomAutomationType getInstance() {
		if (instance == null)
			instance = new RandomAutomationType();
		return instance;
	}
	...
	public boolean automate(PostGenerationAction action,
		...
		if (action instanceof FailAction) {
			...
			if(activate && v instanceof TreeFeature && !((TreeFeature)f).getVersions().isEmpty()){
			...
			simulationAccessor.setFeatureActivated(
					((GetFeatureStateAction) action).getFeature(),v,
					activate);
			return true;
